...
def extract_comment_text(raw_detail: str) -> str:
    ...
    Example:
        "גל אנתבי 10-891-003641132 ניקיון חודשי"  →  "ניקיון חודשי"
        "ניקיון ללא מספר"                          →  "ניקיון ללא מספר"
    """
    if not isinstance(raw_detail, str):
        return ""
    # Bank account numbers look like: 10-891-003641132
...
def _load_category_rules(csv_path: str = "categories.csv") -> list[dict]:
    ...
    Tries the given path first, then the directory of this file.
    Raises FileNotFoundError loudly so a missing file is never silently ignored.
    """
    import csv
    from pathlib import Path

    p = Path(csv_path)
    # Fallback: look next to bank_parser.py itself
    if not p.exists():
...
def _match_category(desc: str, ext_desc: str, rules: list[dict]) -> tuple[str | None, str | None]:
    """
    Try each keyword rule against the appropriate column.
    Returns (category, entity_name) for the first match, or (None, None).
    """
    for rule in rules:
        kw = rule["keyword"]
        if not kw:
...
def parse_debit_file(path: str, categories_csv: str = "categories.csv") -> list[dict]:
    ...
    Each returned dict:
    {
        ...
        "amount":      350.0,
        "raw_detail":  "...",
        "category":    "חשמל ציבורי" | None,
        "entity_name": "חברת החשמל"  | None,
        "match_method":"keyword" | "unmatched",
    }
    """
